...
namespace MikuMikuWorld
{
	...
	void ScoreEditor::update()
	{
		...
		if (!ImGui::GetIO().WantCaptureKeyboard)
		{
			...
			if (ImGui::IsAnyPressed(config.input.trySaveSus)) trySaveSus(context.workingData.filename, context.workingData.susfilename);
	...
	bool ScoreEditor::save(std::string filename)
	{
		try
		{
			...
			//ägí£éqçûÇ›Ç»ÇÃÇ≈ÅAä‹Ç‹Ç»Ç¢ÇÊÇ§Ç…ï∂éöóÒÇçÌÇ¡ÇƒÇ©ÇÁägí£éqí«â¡
	...
	bool ScoreEditor::trySaveSus(std::string filename, std::string susfilename)
	{
		...
		if (susfilename.empty())
			exportSus();
		else
			return saveSus(susfilename);

		return false;
	}


	bool ScoreEditor::saveSus(std::string susfilename)
	{
		try
		{
			SUS sus = ScoreConverter::scoreToSus(context.score);
			context.workingData.susfilename = susfilename;
			...
			exporter.dump(sus, susfilename, exportComment);
			context.upToDate = true;
		}
	...
	void ScoreEditor::exportSus()
	{
		...
		if (fileDialog.saveFile() == IO::FileDialogResult::OK)
		{
			...
			{
				SUS sus = ScoreConverter::scoreToSus(context.score);
				context.workingData.susfilename = fileDialog.outputFilename;
				const std::string exportComment = IO::concat("This file was generated by " APP_NAME, Application::getAppVersion().c_str(), " ");
				SusExporter exporter;
	...
	void ScoreEditor::drawMenubar()
	{
		...
		if (ImGui::BeginMenu(getString("file")))
		{
			...
			if (ImGui::MenuItem(getString("save_Sus"), ToShortcutString(config.input.trySaveSus)))
				trySaveSus(context.workingData.filename, context.workingData.susfilename);
