...
namespace MikuMikuWorld
{
	...
	void ScoreEditor::update()
	{
		...
		if (!ImGui::GetIO().WantCaptureKeyboard)
		{
			...
			if (ImGui::IsAnyPressed(config.input.trySaveSus)) trySaveSus(context.workingData.filename);
	...
	bool ScoreEditor::trySaveSus(std::string filename)
	{
		if (filename.empty())
			exportSus();
		else
			return saveSus(filename);

		return false;
	}


	bool ScoreEditor::saveSus(std::string filename)
	{
		try
		{
	...
	void ScoreEditor::drawMenubar()
	{
		...
		if (ImGui::BeginMenu(getString("file")))
		{
			...
			if (ImGui::MenuItem(getString("save_Sus"), ToShortcutString(config.input.trySaveSus)))
				trySaveSus(context.workingData.filename);
